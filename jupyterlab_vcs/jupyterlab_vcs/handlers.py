...
from .github_v3 import (
    create_pull_request,
    get_repository,
    get_repository_details_for_pr,
)
from .utils import (
    execute_git_command,
    get_branches,
...
class RouteHandler(APIHandler):
    ...
    def get(self):
        self.finish(
            json.dumps({"data": "This is /jupyterlab-vcs/get-example endpoint!"})
        )
    ...
    def post(self):
        ...
        try:
            repo_path = os.path.join(
                os.getcwd(), os.path.basename(repo_url).replace(".git", "")
            )
        ...
        try:
            ...
            if not os.path.exists(os.path.join(repo_path, ".git")):
                ...
                self.finish(
                    json.dumps(
                        {
                            "error": "No Git repository found. Please clone a Git project first."
                        }
                    )
                )
                return
...
class PullRequestHandler(APIHandler):
    ...
    def post(self):
        ...
        if not commit_message or not pr_title or not branch:
            ...
            self.finish(
                json.dumps(
                    {"error": "Commit message, PR title, and branch are required"}
                )
            )
            return
        ...
        try:
            ...
            details = get_repository_details_for_pr(
                owner_login, repo_name, token, branch
            )
            result = create_pull_request(
                *details, pr_title, branch, base_branch, token, "https://pranay.com/"
            )
...
class CommitHandler(APIHandler):
    ...
    def post(self):
        ...
        try:
            ...
            if not token:
                raise Exception(
                    "No GitHub token configured. Please configure a token using Configure Git."
                )
            username = _user_store.get(repo_path)
            if not username:
                raise Exception(
                    "No GitHub username configured. Please configure a username using Configure Git."
                )
            ...
            logger.warning(
                f"Executing: git commit -m '{commit_message}' in {repo_path}"
            )
            execute_git_command(repo_path, "commit", "-m", commit_message, env=env)

            logger.warning(
                f"1 Executing: git commit -m '{commit_message}' in {repo_path}"
            )
            result = {
            ...
            logger.warning(
                f"2 Executing: git commit -m '{commit_message}' in {repo_path}"
            )
            self.finish(json.dumps(result))
            logger.warning(
                f"3 Executing: git commit -m '{commit_message}' in {repo_path}"
            )
        except Exception as e:
    ...
    def post(self):
        try:
            ...
            if not os.path.exists(os.path.join(repo_path, ".git")):
                ...
                return
            modified_files = (
                execute_git_command(repo_path, "status", "--porcelain")
                .strip()
                .splitlines()
            )
            repos = [
                {"name": os.path.basename(repo_path), "path": repo_path}
